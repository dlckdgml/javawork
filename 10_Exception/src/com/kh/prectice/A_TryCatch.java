 ...
 * 런타일 에러(runtime error) : 실행시에 발생하는 에러(빨간줄은 확인 안되지만 콘솔창에서 오류내영 확인 가능)
 * 논리적 에러(logical error) : 실행은 되지만, 의도와 다르게 동작함(코드를 잘못짜서 다시 짜야함)
 * 
 * 런타임 에러의 종류 2가지 
 * 에러(error) : 프로그램 코드에 의해서 수습될 수 없는 심간한 오류
 * --> 메모리 부족(OutOfMememortError)또는 스택오버플로우(StackOverflowError)
...
public class A_TryCatch {
	...
	/*
	 * try ~ catch [표현법] try { // 예외가 발생할 가능성이 있는 코드 } catch(예외클래스 e) { // try 블록에서
	 * 예외가 발생할 때 --> catch블록으로 이동하여 예외처리 코드 실행 } finally { // finally는 있어도 되고 없어도 됨
	 * // 예외가 발생하거나 발생하지 않거나 무조건 실행 // finally 주로 자원을 반납하는 코드 ... close() }
	 *
