...
/*
 * java.time 패키지
 * - Date와 Calendar의 단점을 개선한 새로운 클래스들을 제공한다(JDK1.8에서 제공함)
...
public class B_Time {
	...
	public void method1() {
		...
		// 날짜 지정해서 출력하기
		LocalDateTime when = LocalDateTime.of(2023, 11, 1, 17, 20, 0); // 연도,월,일,시,분,초
		...
		System.out.println("localDate : " + localDate); // 년-월-일만 출력됨
		localDate = LocalDate.of(2023, 11, 1); // 괄호 년, 월, 일로 날짜로 변경
		...
		localTime = LocalTime.of(9, 27, 34); // 괄호안에 시, 분, 초로 변경
	...
	public void method2() {
		...
		System.out.println("days : " + pe.getDays()); // 일 차이마나 출력 위에서 30D에 해당함
		...
		System.out.println("duration : " + du); // PT10H36M46S 로 출력됨
	...
	public void method3() {
		...
		// 형식이 맞지 않아 출력 시 오류 발생함 DateTimeFormatter.ofPattern()로 형식을 지정하면 오류 안생김
		System.out.println(date);
		date = LocalDate.parse("20230511", DateTimeFormatter.BASIC_ISO_DATE); 
		// 역시 형식이 맞지 않아 오류가 발생하고 DateTimeFormatter.BASIC_ISO_DATE를 작성해야 오류 안생김
		System.out.println(date);
		...
		// ZonedDateTime Format ~ 아래 에시 외에도 다양한 포맷들이 있음
	...
	public void method4() {
		...
		dDay = nowDate.until(inputDate, ChronoUnit.DAYS); // 윗 줄과 내용
